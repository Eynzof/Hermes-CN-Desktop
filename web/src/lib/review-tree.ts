// Pure changed-file tree builders for the review pane (issue #328). Ported from
// the upstream `right-sidebar/review/tree-data.ts`; kept free of React so the
// flat-list / folder-hierarchy shaping is unit testable in isolation.
...
export function buildReviewFlatList(files: ReviewFile[]): ReviewTreeNode[] {
  return [...files]
...
export function buildReviewTree(files: ReviewFile[], compact = true): ReviewTreeNode[] {
  const root = makeDir("", "");
