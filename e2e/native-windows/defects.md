...
| WIN-026 | 本地模型连接误报 | 本地部署表单明确允许不填 API Key，实际无密钥模型列表和真实生成均成功，“测试连接”却显示 API Key 被拒绝。Core provider.probe 在解析地址和发起网络请求前直接拒绝空 Key。 | MODEL-009 runs/20260907T121230Z：探测软失败，随后本地 65536 上下文真实调用及切回官方 DeepSeek 均完成；未修复 |
| WIN-027 | 默认模型与会话状态不同步 | 设置页已选本地模型，121230Z 实际本地生成时会话标题栏和选择器仍显示 DeepSeek。123050Z 新会话甚至实际调用旧的 DeepSeek；123301Z 的会话选择器搜索当前本地模型为空。 | 分别保存真实计费来源、设置页状态、选择器截图和会话证据；显示陈旧不等于请求一定走错，两种情况区分记录。原因为何影响预建会话仍待定位；未修复 |
| WIN-028 | 本地原生识图与工具状态 | 122527Z 配置强制原生输入且实际主模型为本地 Qwen3.5，日志仍尝试自动视觉后端并报未配置；随后模型未能读图而寻找工具。同轮辅助视觉成功读出随机像素。后续不重载的 123050Z / 123301Z 辅助会话工具集中缺少 vision_analyze。 | 不是模型没有视觉能力：同一模型实际辅助识图已通过。原生输入能力判断和工具列表状态需分别定位；失败与成功边界见 local-vision-coverage-audit.md，未修复 |
...
| TEST-038 | 本地模型控件定位与失败清理 | ModelCombobox 候选实际在 dialog 外的 portal；模型列表缓存后按钮文本变为“已加载 N 个”，不能继续按“刷新模型列表”查找；Escape 会直接关闭整个表单。 | 改为按实际 option、稳定 title 和可见取消按钮操作；保留主错误。121230Z 完整执行本地推理和恢复，探测失败属于 WIN-026 |
| TEST-039 | 视觉专项等待与失败管理 | 首次原生识图发现非预期工具后由人工点停止；123533Z 重载后立即读取 API，早于 runtime/bridge 初始化。 | 原生用例现监听真实持久工具调用并通过 UI 自动停止，后续阶段继续；重载等待 backendReady 和 bridge。探索记录保留，不把人工停止或脚本初始化异常当作原生识图成功 |
| TEST-040 | 专项配置清理不完整 | MODEL-009 / CHAT-013 切回 DeepSeek 时沿用本地模型的 65536 上下文，未恢复原先的 1048576。独立环境核查又误读 legacy custom_providers，当前服务商实际在 providers 映射。 | 三份测试前备份确认原始 1M，已从 UI 恢复。125301Z 两个脚本均验证上下文恢复为 1048576；图片脚本同时核对真实 providers 映射中的删除结果。属于测试设施问题，不归为产品缺陷 |
