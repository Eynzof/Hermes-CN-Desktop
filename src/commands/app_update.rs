...
fn run_windows_updater_helper(args: &[OsString], log_path: &Path) -> Result<(), String> {
    ...
    let install_error = match status {
        Ok(status) if status.success() => None,
        Ok(status) => Some(format!("Windows updater 安装失败：{status}")),
    ...
    if let Some(error) = install_error {
        ...
        return Err(format!("{error}{restart_note}"));
    }
    let child = restart?;
...
mod tests {
    ...
    #[cfg(target_os = "windows")]
    #[test]
    fn windows_helper_restarts_desktop_when_installer_is_locked() {
        use base64::Engine as _;
        use std::os::windows::fs::OpenOptionsExt;

        let temp = tempfile::tempdir().unwrap();
        let system32 = PathBuf::from(std::env::var_os("WINDIR").unwrap()).join("System32");
        let cmd = system32.join("cmd.exe");
        let powershell = system32.join("WindowsPowerShell/v1.0/powershell.exe");
        let installer = temp.path().join("locked-installer.exe");
        let restored = temp.path().join("restored.txt");
        let script = format!(
            "[IO.File]::WriteAllText('{}','recovered')",
            restored.display().to_string().replace('\'', "''")
        );
        ...
        let error = run_windows_updater_helper(&args, &temp.path().join("helper.log"))
            .expect_err("locked installer must still report failure");
        assert!(error.contains("运行 Windows updater 失败"), "{error}");
        let deadline = Instant::now() + Duration::from_secs(5);
        while !restored.exists() && Instant::now() < deadline {
            std::thread::sleep(Duration::from_millis(20));
        }
        assert_eq!(fs::read_to_string(restored).unwrap().trim(), "recovered");
    }
