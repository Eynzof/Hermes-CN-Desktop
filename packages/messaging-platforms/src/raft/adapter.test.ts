...
describe("RaftAdapter", () => {
  const config = raftConfigSchema.parse({
    enabled: true,
  });

  it("starts and stops without credentials", async () => {
    const adapter = new RaftAdapter(config);
    expect(adapter.platform).toBe("raft");
  ...
  it("sends text messages", async () => {
    ...
    expect(res.messageId).toMatch(/^raft_/);
  });
  ...
  it("skips connect when disabled", async () => {
    const disabled = raftConfigSchema.parse({ enabled: false });
    const adapter = new RaftAdapter(disabled);
    await adapter.connect();
