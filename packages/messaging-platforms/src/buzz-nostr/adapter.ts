/**
 * Buzz (Nostr) platform adapter v1 surface.
 *
...
export const buzzNostrConfigSchema = z.object({
  enabled: z.boolean().default(false),
  relayUrl: z.string().optional(),
  privateKey: z.string().optional(),
  transport: z.enum(["auto", "websocket", "poll"]).default("auto"),
  channels: z.array(z.string()).optional(),
  allowedUsers: z.array(z.string()).optional(),
  commandPrefix: z.string().default("/"),
});
export type BuzzNostrConfig = z.infer<typeof buzzNostrConfigSchema>;

export class BuzzNostrAdapter implements PlatformAdapter {
  status: PlatformStatus = "idle";
  private config: BuzzNostrConfig;
  private messageCounter = 0;

  readonly platform = "buzz-nostr";

  constructor(config: BuzzNostrConfig) {
    this.config = buzzNostrConfigSchema.parse(config);
  }
  ...
  async send(chatId: string, content: OutboundContent, _meta?: SendMeta): Promise<SendResult> {
    ...
    return { ok: true, messageId: `buzz-nostr_${this.messageCounter}_${chatId}` };
  }

  async sendDocument(chatId: string, _path: string, _meta?: SendMeta): Promise<SendResult> {
    return { ok: true, messageId: `buzz-nostr_doc_${chatId}` };
  }

  async sendImageFile(chatId: string, _path: string, _meta?: SendMeta): Promise<SendResult> {
    return { ok: true, messageId: `buzz-nostr_img_${chatId}` };
  }
