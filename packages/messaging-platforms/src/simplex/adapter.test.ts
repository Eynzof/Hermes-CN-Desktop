...
describe("SimplexAdapter", () => {
  const config = simplexConfigSchema.parse({
    enabled: true,
  });

  it("starts and stops without credentials", async () => {
    const adapter = new SimplexAdapter(config);
    expect(adapter.platform).toBe("simplex");
  ...
  it("sends text messages", async () => {
    ...
    expect(res.messageId).toMatch(/^simplex_/);
  });
  ...
  it("skips connect when disabled", async () => {
    const disabled = simplexConfigSchema.parse({ enabled: false });
    const adapter = new SimplexAdapter(disabled);
    await adapter.connect();
