/**
 * Home Assistant (messaging) platform adapter v1 surface.
 *
...
export const homeassistantMessagingConfigSchema = z.object({
  enabled: z.boolean().default(false),
  url: z.string().default("http://homeassistant.local:8123"),
  token: z.string().optional(),
  watchDomains: z.array(z.string()).optional(),
  watchEntities: z.array(z.string()).optional(),
  ignoreEntities: z.array(z.string()).optional(),
  watchAll: z.boolean().default(false),
  cooldownSeconds: z.number().default(30),
  commandPrefix: z.string().default("/"),
});
export type HomeassistantMessagingConfig = z.infer<typeof homeassistantMessagingConfigSchema>;

export class HomeassistantMessagingAdapter implements PlatformAdapter {
  status: PlatformStatus = "idle";
  private config: HomeassistantMessagingConfig;
  private messageCounter = 0;

  readonly platform = "homeassistant-messaging";

  constructor(config: HomeassistantMessagingConfig) {
    this.config = homeassistantMessagingConfigSchema.parse(config);
  }
  ...
  async send(chatId: string, content: OutboundContent, _meta?: SendMeta): Promise<SendResult> {
    ...
    return { ok: true, messageId: `homeassistant-messaging_${this.messageCounter}_${chatId}` };
  }

  async sendDocument(chatId: string, _path: string, _meta?: SendMeta): Promise<SendResult> {
    return { ok: true, messageId: `homeassistant-messaging_doc_${chatId}` };
  }

  async sendImageFile(chatId: string, _path: string, _meta?: SendMeta): Promise<SendResult> {
    return { ok: true, messageId: `homeassistant-messaging_img_${chatId}` };
  }
