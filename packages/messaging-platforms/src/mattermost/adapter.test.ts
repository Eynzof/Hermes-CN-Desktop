...
describe("MattermostAdapter", () => {
  const config = mattermostConfigSchema.parse({
    enabled: true,
    token: "test-value",
    webhookSecret: "shhh",
  });
  ...
  it("sends text messages", async () => {
    ...
    expect(res.messageId).toMatch(/^mattermost_/);
  });
  ...
  it("short-circuits when the webhook secret is unset", () => {
    const noSecret = mattermostConfigSchema.parse({ enabled: true });
    const adapter = new MattermostAdapter(noSecret);
  ...
  it("skips connect when disabled", async () => {
    const disabled = mattermostConfigSchema.parse({ enabled: false });
    const adapter = new MattermostAdapter(disabled);
    await adapter.connect();
