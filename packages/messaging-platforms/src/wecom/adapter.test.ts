...
describe("WeComAdapter", () => {
  const config = weComConfigSchema.parse({
    enabled: true,
    corpId: "test-value",
    webhookSecret: "shhh",
  });
  ...
  it("sends text messages", async () => {
    ...
    expect(res.messageId).toMatch(/^wecom_/);
  });
  ...
  it("short-circuits when the webhook secret is unset", () => {
    const noSecret = weComConfigSchema.parse({ enabled: true });
    const adapter = new WeComAdapter(noSecret);
  ...
  it("skips connect when disabled", async () => {
    const disabled = weComConfigSchema.parse({ enabled: false });
    const adapter = new WeComAdapter(disabled);
    await adapter.connect();
