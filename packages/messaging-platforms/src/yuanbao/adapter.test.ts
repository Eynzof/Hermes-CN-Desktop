...
describe("YuanbaoAdapter", () => {
  const config = yuanbaoConfigSchema.parse({
    enabled: true,
  });

  it("starts and stops without credentials", async () => {
    const adapter = new YuanbaoAdapter(config);
    expect(adapter.platform).toBe("yuanbao");
  ...
  it("sends text messages", async () => {
    ...
    expect(res.messageId).toMatch(/^yuanbao_/);
  });
  ...
  it("skips connect when disabled", async () => {
    const disabled = yuanbaoConfigSchema.parse({ enabled: false });
    const adapter = new YuanbaoAdapter(disabled);
    await adapter.connect();
